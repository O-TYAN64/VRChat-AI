...
WAKE_WORDS = [
    w.strip() for w in os.getenv("WAKE_WORD", "えーあい,AI,あい").split(",")
]
...
#やってはいけないこと
- 行動() や 「」 を使って行動や感情の表現をする
- 箇条書き
- 教師口調
- 尋問のような質問
- 機械的な返答

このルールを守って、自然な会話をしてください。
"""
...
def build_messages_with_limit(user_text: str, system_text: str) -> list:
    ...
    if total_tokens + ut <= MAX_CONTEXT_TOKENS:
        ...
        messages.append({
            "role": "user",
            "content": user_text[-MAX_CONTEXT_TOKENS:]
        })
