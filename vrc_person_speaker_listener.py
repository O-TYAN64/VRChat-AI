...
def save_speech(speaker, content):
    ...
    # 話者プロファイル更新
    cur.execute("SELECT speak_count FROM speaker_profiles WHERE speaker=?", (speaker,))
    row = cur.fetchone()
    ...
    if row:
        ...
        VALUES (?, 1, ?, ?)
        """, (speaker, content, "まだ判断できない"))
...
def find_vrchat_window():
    ...
    def callback(hwnd, _):
        if not win32gui.IsWindowVisible(hwnd):
            return
        if win32gui.GetWindowText(hwnd) == "VRChat" and win32gui.GetClassName(hwnd) == "UnityWndClass":
            result.append(hwnd)
...
def yolo_run():
    ...
    while True:
        ...
        for box in results[0].boxes or []:
            ...
            if is_speaking(plate):
                name = extract_name(plate)
                if not name:
                    continue
                ...
                if name not in last_capture or time.time() - last_capture[name] > 3:
                    ...
                    if content:
                        ...
                        print(f"💬 {name}: {content}")
                    last_capture[name] = time.time()

                cv2.putText(frame, f"SPEAKING: {name}",
                            (x1, y1 - 10),
